...
-(id)initwithUUChartDataFrame:(CGRect)rect withSource:(id<UUChartDataSource>)dataSource withStyle:(UUChartStyle)style isSpecial:(BOOL)isSpecial arrColor:(NSArray*)arrColor{
...
-(void)setUpChart{
	...
	}else if (self.chartStyle == UUChartBarStyle)
	{
        if (!_barChart) {
            if (self.isSpecial) {
                _barChart = [[UUBarChart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) isSpecial:self.isSpecial arrColor:self.arrColor];
            }else
                _barChart = [[UUBarChart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
            
            _barChart.xLblIsAlighLeft = self.xLblIsAlighLeft;
