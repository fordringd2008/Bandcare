...
- (void)setShowPercentage:(BOOL)showPercentage
{
    ...
    {
        CATextLayer *textLayer = [(NSArray *)[layer sublayers] objectAtIndex:0];
...
//        CGSize size = [label sizeWithFont:self.labelFont];
        ...
        NSDictionary * tdic = [NSDictionary dictionaryWithObjectsAndKeys:self.labelFont, NSFontAttributeName,nil];
        size =[label boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin |NSStringDrawingUsesFontLeading attributes:tdic context:nil].size;
...
- (void)setSliceSelectedAtIndex:(NSInteger)index
{
    if(_selectedSliceOffsetRadius <= 0)
        return;
    SliceLayer *layer = [(NSArray *)(_pieView.layer.sublayers) objectAtIndex:index];
    if (layer) {
        float adjust = 0.5;
...
- (void)reloadData
{
    if (_dataSource && !_animationTimer)
    {
        ...
        _selectedSliceIndex = self.indexFirst;
...
{
//    [self notifyDelegateOfSelectionChangeFrom:_selectedSliceIndex to:selIndex]; // 选中后的 下滑
...
- (void)notifyDelegateOfSelectionChangeFrom:(NSUInteger)previousSelection to:(NSUInteger)newSelection
{
    ...
    {
        SliceLayer *layer = [(NSArray *)_pieView.layer.sublayers objectAtIndex:newSelection];
...
//    CGSize size = [@"0" sizeWithFont:self.labelFont];
    ...
    CGSize size = CGSizeMake(320,2000); //设置一个行高上限
    NSDictionary * tdic = [NSDictionary dictionaryWithObjectsAndKeys:self.labelFont, NSFontAttributeName,nil];
    size =[@"0" boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin |NSStringDrawingUsesFontLeading attributes:tdic context:nil].size;
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value
{
    CATextLayer *textLayer = [(NSArray *)[pieLayer sublayers] objectAtIndex:0];
    [textLayer setHidden:!_showLabel];
    if(!_showLabel) return;
    NSString *label;
    if(_showPercentage)
